"""Task 6 — PPO + ``inv:detoxify`` evaluation [5 points].

Eval the merged PPO checkpoint trained with ``TOXIC_REWARD=inv:detoxify``.
Re-uses the eval helpers; the work for this task is in
``submissions/notes.md`` — what happened, did it collapse, what does
the attractor look like?
...
def main() -> None:
    ...
    print("\nPPO (inv:detoxify) — greedy mean Detoxify per slice:")
    for k_, v in greedy.items():
        print(f"  {k_:20s}  {v:.3f}")
    print("\nPPO (inv:detoxify) — K=16 diagnostic:")
    ...
    print("\nPPO (inv:detoxify) — worst-of-16 on mild_prefix (first 3 shown):")
    ...
    out.write_text(json.dumps({
        "model": "ppo_inv_detoxify", "ppo_dir": str(a.ppo_dir),
